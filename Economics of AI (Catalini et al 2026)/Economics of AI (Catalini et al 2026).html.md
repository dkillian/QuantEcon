...
subtitle: "Notes on selected concepts"
...
## Background: The Romer Endogenous Growth Model
...
::: {.cell-output-display}
![](Economics-of-AI--Catalini-et-al-2026-_files/figure-html/romer-sim-1.png){width=1056}
:::
:::


### Departures in Catalini et al.

Catalini et al. retain Romer's aggregate Cobb-Douglas form but make three substantive changes:

1. **The composition of effective labor.** The single production aggregate $L_Y$ is replaced by a Cobb-Douglas sub-aggregate of *measurable capacity* $L_m$ and *non-measurable capacity* $L_{nm}$ — decomposing work along the axis of verifiability rather than skill.
2. **Agentic labor and the verifiable share.** Agentic labor $L_a$ enters $L_m$ as a perfect substitute for human execution, but only the fraction that survives human verification — the *verifiable share* $s_v$ — contributes to valid output. Unverified agentic output does not disappear; it leaks into the economy as a resource-consuming externality $X_A$.
3. **Endogenous knowledge extraction.** Knowledge accumulation gains a tacit-extraction channel: every hour experts spend verifying agents also generates proprietary training data $K_{IP}$ that endogenously lowers the future cost of automation — the Codifier's Curse.

These modifications reorient Romer's model from a story about knowledge *creation* to a story about knowledge *verification*.
